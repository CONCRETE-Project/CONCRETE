...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\CONCRETE
// Windows >= Vista: C:\Users\Username\AppData\Roaming\CONCRETE
// Mac: ~/Library/Application Support/CONCRETE
// Unix: ~/.concrete
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CONCRETE";
#else
    fs::path pathRet;
...
{
    fs::path pathConfigFile(GetArg("-conf", "concrete.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty concrete.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override concrete.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", "concreted.pid"));
