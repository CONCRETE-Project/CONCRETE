...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"zCCEsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zCCE denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zCCE denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zCCE denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zCCE denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zCCE denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zCCE denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zCCE denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zCCE denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zCCE denominations\n"
            "  }\n"
    ...
    UniValue zcceObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        if (mapZerocoinSupply.empty())
            zcceObj.push_back(Pair(std::to_string(denom), ValueFromAmount(0)));
        else
            zcceObj.push_back(Pair(std::to_string(denom), ValueFromAmount(mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zcceObj.push_back(Pair("total", ValueFromAmount(GetZerocoinSupply())));
    obj.push_back(Pair("zCCEsupply", zcceObj));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"cceaddress\"\n"
            "\nReturn information about the given concrete address.\n"

            "\nArguments:\n"
            "1. \"cceaddress\"     (string, required) The concrete address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"cceaddress\",    (string) The concrete address validated\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"cceaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"cceaddress\"  (string, required) The concrete address to use for the signature.\n"
