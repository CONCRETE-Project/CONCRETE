...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total concrete balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zCCTsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zCCT denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zCCT denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zCCT denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zCCT denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zCCT denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zCCT denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zCCT denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zCCT denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zCCT denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,      (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,          (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,        (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,           (numeric) the transaction fee set in concrete/kb\n"
            "  \"relayfee\": x.xxxx,           (numeric) minimum relay fee for non-free transactions in concrete/kb\n"
    ...
    UniValue zcctObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        if (mapZerocoinSupply.empty())
            zcctObj.push_back(Pair(std::to_string(denom), ValueFromAmount(0)));
        else
            zcctObj.push_back(Pair(std::to_string(denom), ValueFromAmount(mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zcctObj.push_back(Pair("total", ValueFromAmount(GetZerocoinSupply())));
    obj.push_back(Pair("zCCTsupply", zcctObj));
...
UniValue mnsync(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1 || (strMode != "status" && strMode != "reset" && strMode != "next")) {
        throw std::runtime_error(
            ...
            "\nReturns the sync status or resets sync or move to next asset.\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"cctaddress\"\n"
            "\nReturn information about the given concrete address.\n"

            "\nArguments:\n"
            "1. \"cctaddress\"     (string, required) The concrete address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"cctaddress\",    (string) The concrete address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for CONCRETE cold staking\n"
...
#ifdef ENABLE_WALLET
        // Case 1: CONCRETE address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are concrete addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) concrete address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"cctaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"cctaddress\"  (string, required) The concrete address to use for the signature.\n"
