...
#include "qt/concrete/loadingdialog.h"
...
void AskPassphraseDialog::accept()
{
    ...
    case Mode::Encrypt: {
        ...
        bool ret = openStandardDialog(
                ...
                " <b>" + tr("LOSE ALL OF YOUR CCT") + "</b>!<br><br>" + tr("Are you sure you wish to encrypt your wallet?"),
                tr("ENCRYPT"), tr("CANCEL")
        );
        if (ret) {
            if (newpass1 == newpass2) {
                newpassCache = newpass1;
                CONCRETEGUI* window = static_cast<CONCRETEGUI*>(parentWidget());
...
{
    CONCRETEGUI* gui = static_cast<CONCRETEGUI*>(parentWidget());
...
{
    hide();
    static_cast<CONCRETEGUI*>(parentWidget())->showHide(true);
    openStandardDialog(
            tr("Wallet encrypted"),
            "<qt>" +
            tr("CONCRETE will close now to finish the encryption process. "
               "Remember that encrypting your wallet cannot fully protect "
               "your CCTs from being stolen by malware infecting your computer.") +
            "<br><br><b>" +
