...
#include "qt/concrete/forms/ui_dashboardwidget.h"
#include "qt/concrete/sendconfirmdialog.h"
#include "qt/concrete/txrow.h"
...
DashboardWidget::DashboardWidget(CONCRETEGUI* parent) :
    PWidget(parent),
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &CONCRETEGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &CONCRETEGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &CONCRETEGUI::openFAQ);
