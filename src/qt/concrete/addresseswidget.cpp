...
#include "qt/concrete/forms/ui_addresseswidget.h"
#include "qt/concrete/addresslabelrow.h"
#include "qt/concrete/addnewaddressdialog.h"
#include "qt/concrete/tooltipmenu.h"

#include "qt/concrete/addnewcontactdialog.h"
...
AddressesWidget::AddressesWidget(CONCRETEGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter CONCRETE address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress cctAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(cctAdd.Get(), label.toUtf8().constData(),
                cctAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
