...
#include "qt/concrete/forms/ui_snackbar.h"
#include "qt/concrete/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(CONCRETEGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &CONCRETEGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
