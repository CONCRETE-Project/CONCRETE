...
CONCRETE Core v3.0.2 is a recommended, semi-mandatory update for all users. This release contains transaction creation bug fixes for zCCE spends, automint calculation adjustments, and other various updates/fixes.

zCCE spending requires this update.
...
Auto Wallet Backup
---------------------
In addition to the automatic wallet backup that is done at each start of the client, a new automatic backup function has been added that will, by default, create a backup of the wallet file during each zCCE mint operation (zCCE spends which re-mint their change are also included in this). This functionality is controlled by the `-backupzcce` command-line option, which defaults to `1` (enabled, auto-backup).

Users that wish to prevent this behavior (not recommended) can pass `-backupzcce=0` at the command-line when starting the client, or add `backupzcce=0` to their `concrete.conf` file.

zCCE Automint Calculations
---------------------
A bug in the automint calculations was made apparent on mainnet when block times exceeded expectations, resulting in zCCE mint transactions that were in an unconfirmed state to still be treated as if they had never been minted. This caused automint to effectively mint more than what was intended.

zCCE Spending Fix
---------------------
The size of zCCE spend transactions is knowingly larger than normal transactions, and while this was expected, a much stricter check against the scriptsig size is used for mainnet, causing the transactions to be rejected by the mempool, and thus not being packaged into any blocks.

zCCE Transaction Recovery
---------------------
Due to the aforementioned issue with zCCE spending, users may find that their attempted spends are now conflicted and zCCE balances are not represented as expected. "Recovery" of these transactions can be done using the following methods:
...
- #292 `feadab4` Additional checks for double spending of zCCE serials. (presstab)

### Wallet
- #271 `5e9a086` [Wallet] Remove unused member wallet in UnlockContext inner class (Jon Spock)
- #279 `e734010` Add -backupzcce startup flag. (presstab)
- #280 `fdc182d` [Wallet] Fix zCCE spending errors. (presstab)
- #282 `310f216` [Wallet] Count pending zCCE balance for automint. (presstab)
...
- #291 `cb314e6` [Qt] zCCE control quantity/amount fixes (rejectedpromise)
