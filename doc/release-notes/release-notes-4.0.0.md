...
The default split threshold remains at 2000 CCE, and can be adjusted in the GUI's Settings page, or via the RPC `setstakesplitthreshold` command.
...
### V1 zCCE Spending (Public Spends Version 4)

Since the discovery of a critical exploit within the libzerocoin library in early 2019, remaining legacy v1 zCCE have been un-spendable. We're happy to say that, once the new consensus rules are in effect, users will once again be able to spend their v1 zCCE with public spends version 4 ([PR #936](https://github.com/CONCRETE-Project/CONCRETE/pull/936)).

As with the previous version 3 public spends introduced in core wallet version 3.3.0 (enabling the spending of v2 zCCE), version 4 spends will also be public. A full technical writeup is available on the [CONCRETE Wiki](https://github.com/CONCRETE-Project/CONCRETE/wiki/CoinRandomnessSchnorrSignature).
...
`SPORK_18` (off by default) is used to switch between Version 3 and [Version 4 Public Spends](#v1-zcce-spending-public-spends-version-4). When this spork is active, only version 4 spends are accepted by the network. When it's not, only version 3 spends are accepted.
...
- `delegated_balance` - CCE balance held in P2CS contracts (delegated amount total).
- `cold_staking_balance` - CCE balance held in cold staking addresses.
...
- #936 `12a6b704b6` [zCCE] PublicCoinSpend v4 - Coin Randomness Schnorr Signature (random-zebra)
